...
| ~~`API_URL` `MODEL_NAME` `API_KEY`~~ | **已移除**。模型接入改为在后台「模型厂商」页建档，见 [多套模型配置](#多套模型配置) |
...
**模型配置只有这一个来源，没有 `.env` 兜底。** 没有档案或没配用途路由时，照片分析会明确失败而不是悄悄换一套配置——后者更难排查：路由填错时分析照样「成功」，只是用了别的模型，等发现时已经烧了一批额度。

每条档案是一个厂商：名称、接口地址、密钥，以及各自独立的超时与图片最长边（本地模型和云端服务的合理超时差得远，共用一个值不合适）。名称一旦建好不能改：用途路由按名字引用档案，改名会留下解析不到的悬空引用，因此只能删除后新建。
...
值是厂商名称。三项都留空时没有任何厂商可用，照片分析会直接失败——这是刻意的，没有 `.env` 兜底可回退。厂商列表页会标注每条档案被哪些用途引用，未被引用的明确标「未被引用」。
