...
| 入口 | 方法与路径 | 说明 |
|---|---|---|
| 页面 | `GET/POST /admin/providers` | 一个 `action` 字段区分 create、update、delete 三个动作，其余取值一律 `ParameterError`；列表页的启用模型下拉也走 update，只提交 `active_model` 一个字段 |
| 编辑页 | `GET /admin/providers/<id>/edit` | 单条档案的编辑表单，含逐行的模型清单与行内连通性测试；提交回列表页的 `action=update`；档案不存在返回 404 |
...
三个页面动作共用一个端点而不是拆成三个，是因为它们共用同一套字段校验与错误回显；拆开会让「新建失败」和「编辑失败」各写一遍渲染逻辑。改造前还有一个 `import`（导入当前配置为厂商）动作，随注册表五个兜底键一并移除，`providers()` 现在只识别 create / update / delete，其余取值抛 `ParameterError`。

**编辑做成独立页面而不是列表页里的行内展开表单**：可编辑字段塞进表格行会把本来就偏宽的表格挤爆，而独立页不依赖脚本，禁用 JavaScript 时同样能改。编辑页只回显可改字段，名称是禁用的只读输入框（服务层拒绝改名），密钥占位文本提示留空即保持原值。提交仍走列表页的 `action=update`，因此校验与乐观锁只有一条路径。

**编辑页还提供「高级请求参数」输入框**，填 JSON 对象，原样合并进发给该厂商的请求体，值写 `null` 表示不发送该参数。用于处理各家不通用的扩展参数，例如千问填 `{"enable_thinking": false, "response_format": null}` 关掉思考并免掉结构化输出。非法 JSON 在保存时就被拒绝并给出示例。取舍理由与实测数据见 [03-照片分析模块](03-照片分析模块.md#厂商特有请求参数)。

**切换启用模型在列表页有一个下拉快捷入口，模型清单的完整编辑在编辑页。** 列表页的下拉只在模型多于一项时出现（一个选项的下拉只是让人多点两下才发现没得选），配一个「切换」按钮走普通表单提交，因此禁用脚本也能换。该表单只提交 `provider_id`、`version` 与 `active_model` 三个字段——页面端点按「提交了才改」语义收键，不凭空补齐空值，否则单字段提交会被误判成同时清空其余字段。

**编辑页把模型名、启用选择和连通性测试合并成同一组行**：每行是「单选按钮（启用）+ 模型名输入框 + 测试按钮 + 结论」。早先的做法是一个分号分隔的模型池文本框加一个启用模型下拉，同一批模型名在两个控件里各列一遍；测试又只在列表页，改完得跳回去测、测完再跳回来改。合并后模型名只出现一次，选哪个启用就在它自己那一行，测完直接改、改完直接测。

**单选按钮的值是行序号而不是模型名。** 这样把某行改成别的模型名时选中关系不会失配，前端也不必在输入时同步单选按钮的值。服务端的 `_provider_model_fields()` 按序号从提交上来的同名 `model_entry` 列表里取，序号相对**未过滤**的原始行列表，因此清空中间某行不会让后面行的选中关系错位。清空某行即删除该模型；清空的正好是选中行时留空，交由服务层落回清单第一项。

模型清单的存储格式没变，仍是 `model_name` 里的分号串，逐行只是页面表现，服务层与迁移都不受影响。末尾始终渲染一个空行，禁用脚本时靠它仍能新增一个模型；`admin-providers.js` 的「再加一个模型」默认 `hidden`，只在脚本可用时显示，加行后会重排全部单选按钮的序号。编辑页的行内测试传本页正在填写的地址与该行模型名（密钥仍留空、由服务端取已存值），因此改完不必先保存就能验——保存错配置正是要避免的事。

**启用状态的复选框前必须放一个同名空值 hidden。** 复选框未勾选时浏览器根本不提交该键，服务端无法区分「取消启用」和「本次不涉及启用状态」；页面端点用 `getlist(...)[-1]` 取同名值的最后一个，勾选时拿到复选框的 `"1"`，未勾选时拿到 hidden 的空串。

**列表页每行四个操作按颜色分级**：编辑为主操作用 `button-primary`，停用/启用会影响正在跑的分析用 `button-warning`，删除不可逆用 `button-danger` 描边红，测试连通性无副作用用 `button-secondary`。早先删除按钮写的是 `.is-danger`，但该规则只对 `.icon-button` 有定义，红色实际从未生效，视觉上与停用同色。

**表格里地址与模型名列不能复用 `.photo-path`**：那套省略号截断靠单元格内部的块级 `<a>` 生效，纯文本单元格只会继承 `.photo-table` 的 `white-space: nowrap`，长接口地址会直接溢出压住右侧各列。厂商页改用 `.provider-url` 与 `.provider-models`，限宽并允许 `overflow-wrap: anywhere` 换行。

**新建表单不复用照片筛选页的 `.filter-panel`**：后者九列的宽度按「搜索、分类、日期范围」那组控件定制，字段数不同会让「启用」复选框排进为下拉预留的窄列，且 `.filter-panel input` 的 `margin-top` 会盖掉 `.filter-check input` 的重置，表现为方框与文字错行。厂商页改用自有 `.provider-form` 栅格，复选框用 `.provider-check` 显式横排。
...
列表页的「测试连通性」只测该档案**当前启用的那个模型**：清单里其余模型此刻并不会被调用，一并测会让「我选的这个能不能用」被一堆无关结论淹没。要逐个验证清单里的模型，去编辑页按行测。接口层仍支持一次传入多个模型名（分号分隔）逐个探测，此时只有全部连通才算通过，返回体的 `models` 数组给出逐模型明细；传输层失败与鉴权失败会立即停止剩余探测，因为那是厂商级问题，其余模型必然是同一个结果。单模型时 `message` 保持扁平措辞，不变成汇总格式。

`resolve_endpoint()` 负责地址归一化：仓库里这个值历史上有两种写法，一处把 `API_URL` 当完整端点直接 POST，另一处把它当 `base_url`。因此按 `/chat/completions` 后缀判断，已指向该端点的原样使用，否则补后缀。管理员在页面上填基础地址（通常以 `/v1` 结尾）或完整端点都可以。

页面常显「模型配置只在这里」块，并在没有档案或三个路由全空时给出提示条。措辞方向与改造前相反：**没有档案或没配路由时分析会直接失败**，因为注册表里那套单值兜底配置（`API_URL`、`MODEL_NAME`、`TIMEOUT`、`VLM_MAX_LONG_EDGE`、`API_KEY`）已整体移除。「导入当前配置为厂商」按钮随之删除——它的输入正是那五个键。

**「启用」与「正在被用」是两件独立的事，状态列必须同时回答。** `is_enabled` 只表示这条档案允许被引用；真正决定谁干活的是配置管理页的三个用途路由。只显示启用状态时，每条档案都写着「启用」却可能一条都没接上，用户会以为建档即生效，然后对着一条根本没被引用的档案排查「分析为什么没走新模型」。因此状态列直接把两个维度合成**三态**：`is_enabled=0` 显示灰色「停用」（不会被调用，也不能被路由引用）；启用且被至少一个路由引用显示绿色「使用中」，`cell-note` 列出具体用途（`照片分析` / `展示文案` / `信息面板`，取自 `PROVIDER_ROUTE_LABELS`，按分号顺序 `join`）；启用但无任何路由指向显示「空闲」并提示去用途路由页选上。若已有档案但三个路由全空，页面顶部再给一条指向配置管理页的 `inline-notice`（由 `any_provider_in_use` 判定）。引用关系由 `ModelProviderService.routes_by_provider()` 一次算出厂商名到路由键的映射，路由层 `_providers_context()` 再翻成中文标签写进每条档案的 `purposes`，避免每条档案各查一遍路由。

`admin-providers.js` 只做连通性测试的异步反馈与编辑页的动态加行，增删改停与保存全部走普通表单提交，因此禁用脚本时两个页面依然可用：列表页少了测试按钮，编辑页少了「再加一个模型」（末尾预留的空行仍能新增一个）。列表页的测试请求只传厂商名，不传密钥也不传模型名，由服务端取该档案已存的密钥与当前启用模型；密钥既不出现在页面里，也不在这条请求里往回走一遍。
...
照片旁白路由留空时跟随分析路由；都没有可用档案时直接失败，没有兜底可回退。信息面板优先级简化为 `PANEL_PROVIDER` → `ANALYSIS_PROVIDER`：`PANEL_AI_MODEL` 只是「用哪个模型名」，本身不含地址与密钥，因此**不能单独决定用哪个厂商**（它原先能绑定注册表那套接口配置，那套已不存在）；选中厂商后它仍作为显式模型名覆盖。解析不出候选时传空值，由面板按「模型不可用」回退规则精选。

三个用途都按分号顺序解析完整候选链，但只在连接失败、超时、HTTP 429 和 HTTP 5xx 时转向下一厂商。其他 HTTP 4xx、响应 JSON 错误、字段缺失或模型正文内容错误不降级：照片任务让当前任务按既有重试流程失败，信息面板则立即回退规则精选。该边界避免鉴权与请求参数错误被备用厂商掩盖，也避免内容错误触发重复付费。

照片任务在首次认领时把分析与旁白完整候选链固化到顶层 `provider`，而信息面板在每次请求期动态解析完整面板候选链。信息面板没有后台任务编号，因此不写 `admin_job_events`，实际切换只记录含用途、起止厂商和稳定原因的警告日志。人工智能筛选缓存键包含成功候选的最终接口地址与模型名，两个厂商即使模型同名也不会串缓存。完整快照兼容、照片任务降级事件和密钥边界见 [03-照片分析模块](03-照片分析模块.md#后台任务配置快照)。
...
「兼容模型接口」段已移除：模型接入整体归「模型厂商」页，配置管理页只留用途路由这个开关。只读的系统与安全项集中到最后一个带配置项的标签，不占据日常位置。「配置审计」是不含任何配置项的纯记录标签，排在最末：审计表又宽又长，塞在系统与安全项下面会把那一屏撑乱。它的 `sections` 为空，因此标签上不渲染项数角标（避免显示「0」），未登记配置项的「未分类」兜底段也会跳过它、落到最后一个带配置项的标签。
