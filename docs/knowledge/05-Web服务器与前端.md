...
照片详情页提供两个重新生成入口：`POST /admin/photos/<id>/reanalyze` 重跑完整分析，`POST /admin/photos/<id>/regenerate-narration` 只重写旁白文案。两者都是**新建任务**，与任务页的「重试」不同——重试只作用于同一个任务且要求 `attempts < max_attempts`，尝试次数用尽后界面上再无补救入口。旧业务字段不会被清空，只有新结果成功产出才覆盖；同类任务已在队列中时按去重返回提示，不会重复消耗模型额度。任务页对尝试次数已用尽的失败任务额外给出照片详情页链接。

缩略图由 `MediaService.thumbnail()` 实时生成，长边与质量取自 `THUMBNAIL_MAX_EDGE`（默认 640）与 `THUMBNAIL_QUALITY`（默认 82），可在线调整。原实现固定 300×200，而后台网格单元约 293 px 宽、高清屏需要约 586 px 像素，等于先缩小再放大近三倍，必然模糊。生成时先用 `draft()` 让 libjpeg 直接以缩小比例解码，再用 LANCZOS 缩放：四千像素级原图整幅解码很贵，而翻页时缩略图请求非常密集——实测长边从 300 提到 640（清晰度约 2.9 倍）后单次耗时几乎不变（158 ms → 162 ms）。小图不放大。

响应带 `Cache-Control: private, max-age` 与弱 `ETag`，并支持 `If-None-Match` 条件请求返回 304。校验值由源文件修改时间、体积与影响输出的两个配置共同构成，因此改尺寸或质量后浏览器会自动重新拉取，不会继续用旧缓存里的模糊图。
