...
照片旁白路由留空时跟随分析路由；分析和旁白都没有配置档案时回退「兼容模型接口」里的旧单套配置。信息面板优先级更强调旧行为兼容：`PANEL_PROVIDER` → 旧 `PANEL_AI_MODEL` 配合旧接口 → `ANALYSIS_PROVIDER` → 旧 `MODEL_NAME`。选中 `PANEL_PROVIDER` 时，`PANEL_AI_MODEL` 仍作为显式模型名覆盖。
