...
`/admin/providers`（侧边导航「模型厂商」）维护多套模型接入档案，让公司自建、千问、豆包等配置共存而不互相覆盖。表结构与设计取舍见 [07-数据库与存储](07-数据库与存储.md#model_providers-与-model_provider_audit-表)。

| 入口 | 方法与路径 | 说明 |
|---|---|---|
| 页面 | `GET/POST /admin/providers` | 一个 `action` 字段区分 create、update、delete、import 四个动作 |
| 列表 | `GET /api/admin/providers` | 密钥只给 `api_key_hint` 末四位 |
| 新建 | `POST /api/admin/providers` | 成功返回 201 |
| 更新 | `PATCH /api/admin/providers/<id>` | 请求体含 `version` 走乐观锁 |
| 删除 | `DELETE /api/admin/providers/<id>` | 请求体含 `version` |
| 连通性测试 | `POST /api/admin/providers/test` | 只传 `name` 即可测已存档案，服务端取已存密钥 |

四个页面动作共用一个端点而不是拆成四个，是因为它们共用同一套字段校验与错误回显；拆开会让「新建失败」和「编辑失败」各写一遍渲染逻辑。

**密钥在页面上只显示末四位**，输入框是 `type="password"`，留空表示保持原值——与 `API_KEY` 的只写语义一致。若把空串当成「清空」，用户改一次超时就会顺手把密钥抹掉。

**连通性测试的必要性**：不测的话，地址或密钥填错只能等分析任务失败才发现，而那时已经排了一批任务、每张都要重试三次。测试向端点发一次 `max_tokens=1` 的最小对话请求，超时封顶 20 秒（档案自身可以配到 600 秒，但点一下测试不该把页面挂十分钟）。HTTP 401 与 403 单独表述为「鉴权失败」，其余 4xx 只报状态码——有些兼容实现会对边界参数报 4xx，但那已经说明地址可达、鉴权通过。

`resolve_endpoint()` 负责地址归一化：仓库里这个值历史上有两种写法，批量脚本把 `API_URL` 当完整端点直接 POST，OpenAI 兼容 SDK 分支把它当 `base_url`。两种都在用，因此按 `/chat/completions` 后缀判断，已指向该端点的原样使用，否则补后缀。

页面常显「当前兜底配置」块：表为空时分析链路仍走配置管理页那套单值配置。不写清楚会让人以为「没建档就等于没配模型」，进而重复配一遍。「导入当前配置为厂商」按钮把兜底配置一键建成档案，免掉数据迁移。
...
保存、只重写文案、重新分析和移入回收站四个按钮位于同一操作栏，危险动作靠右；窄屏允许换行并铺满宽度。编辑表单与三个动作表单相互独立，全部保留跨站请求伪造保护。`admin-photo-detail.js` 负责初始脏值比较、提交草稿、轮询、页面恢复和安全回填；保存按钮初始禁用，只有可编辑字段（含两项评分与收录状态）变化后才启用。

### 详情页的上一张 / 下一张

翻页箭头在照片预览框内、左右各一个，视觉与展示页的 `.navigation-hint` 对齐（圆形半透明底、居中于竖向中线、淡入淡出），并且静置约 2.5 秒后自动隐藏、鼠标移动或按键即恢复。后台不引入 Font Awesome，箭头用内联 SVG（Lucide chevron）。两端时置灰保留占位而不是移除元素，否则翻到头时箭头突然消失、位置会跳动。

「下一张」的含义被严格定义为**当前列表里的下一张**：邻居按 `return_query` 携带的筛选与排序计算，因此在筛选结果内翻页不会跳到被筛掉的照片上；没有 `return_query` 时退化为列表页默认口径（`latest`、无筛选），从地址栏直接打开也有可用导航。邻居链接继续带同一个 `return_query`，翻页后「返回列表」和再次翻页仍在同一上下文。

实现分三层，关键在于**筛选与排序的解释只有一处**：

| 层 | 成员 | 职责 |
|---|---|---|
| 仓储 | `_admin_filter_clause()` | 抽出的 WHERE 构造，分页列表与相邻查询共用 |
| 仓储 | `find_adjacent_admin_photos()` | 用 `LAG` / `LEAD` 窗口函数一次算出前后邻居 |
| 服务 | `_normalize_admin_filters()` | 抽出的参数归一化，`list_photos()` 与 `adjacent_photos()` 共用 |

用窗口函数而不是取回整个结果集再找位置：筛选命中上千张时后者会把一次翻页的成本放大到全表。也不手写排序键比较——后台排序表达式含多列与 NULL 优先级（`ADMIN_SORT_EXPRESSIONS`），手写等价条件极易与列表页产生偏差。`JOIN display_stats` 必须保留，`shown_most` / `shown_least` 的排序表达式引用它。排序键仍走列表页那套白名单，不因为来自 `return_query` 就放行。

测试拿列表实际返回的顺序当基准逐项核对邻居，而不是另写一份预期顺序——后者等于把同一个排序逻辑抄第二遍。

### 点图全屏与全屏内翻页

全屏容器是**整个预览框**（`[data-detail-viewer]`）而不是图片按钮：箭头是按钮的兄弟节点，对按钮请求全屏会把箭头留在全屏之外，全屏里就没法翻页了。用原生 Fullscreen API，Esc 退出、层级提升与焦点处理由浏览器负责，不自建遮罩；浏览器不支持时什么都不做，图片本来就正常显示。

**全屏内翻页不能走链接跳转**——导航会让浏览器退出全屏，「全屏连续看图」当场断掉。因此全屏态下脚本拦截箭头点击，改为换 `img.src` 加 `history.replaceState()`，并向 `GET /api/admin/photos/<id>/adjacent` 取新的邻居编号（筛选参数从 `return_query` 透传，与页面链接同一套白名单）。这个接口只为此存在。

退出全屏时若地址栏里的照片已经变了就整页刷新一次：全屏期间只换了图，表单、EXIF 与生命周期区域仍是进入全屏那张的数据，不刷新会出现「图是新的、下面信息是旧的」。非全屏状态下箭头保持普通链接语义，整页跳转，各区域自然跟着更新。

**预览框必须禁止选中。** `.detail-viewer` 带 `user-select: none`、`-webkit-user-select: none` 与 `-webkit-tap-highlight-color: transparent`，图片另加 `user-drag: none`。原因是实测出来的：连续快速点击箭头会被浏览器判成双击/三击并在框内建立选区，而箭头是绝对定位的兄弟节点，选区横跨到中间的图片上，表现为**图片蒙一层系统选中色（蓝色半透明）**；只有点击间隔落进双击判定窗口时才出现，所以偶发、不必现。展示页的同类交互早就有 `user-select: none`，后台复刻这套交互时漏掉了——新增同类浮层交互时要一并照抄这项。箭头的 click 处理里另有 `getSelection().removeAllRanges()` 兜底，清理由其他祖先节点产生的选区。

**连续点击必须排队而不是丢弃。** 邻居查询要一次网络往返，而连续点击的间隔常常更短。早期实现用一个 `navigating` 标志直接 `return`，表现为「点了三下只前进一张」。现在换图与地址改写立刻执行、不等网络，邻居未返回时把点击方向排队（同方向累加，深度上限 5，避免按住不放堆积出一长串跳转），邻居一到就继续消费。

禁用态箭头**不输出** `href` 属性而不是置为空串：`href=""` 在部分浏览器等价于当前页地址，一旦 `pointer-events: none` 或脚本里那道 `is-disabled` 判断失效，点击就会重载当前页。

### 收录状态可在详情页单张修改

`PhotoEditForm.curation` 与批量入口同一套取值命名（`curation` -> `is_included`），走 `update_photo()` 既有的乐观锁与审计路径，审计 `action` 为 `photo_update`。它放在「原始信息修正」分段、城市下方，而不是混在「生成结果」里——收录是人工判断，不是模型产出。同理它属于可编辑字段但**不属于「生成结果」**：重新分析不会覆盖人工收录判断，所以只进 `admin-photo-detail.js` 的 `editableNames`、不进 `generatedNames`；漏掉它会导致改了收录状态而保存按钮仍禁用。

该字段用 `validate_choice=False`，缺失时按「不改动收录状态」处理而非整表校验失败：否则浏览器里缓存的旧详情页一提交就是 400，用户会看到一次莫名的校验错误。取值合法性由服务层 `PhotoManagementService._curation()` 统一把关。

两个评分控件保持相邻（`edit-grid` 是两列，分类与分析状态占第一行，回忆分与美观分占第二行）：它们都是 0 到 100 的数字，改的时候要对照着看。

### 界面称呼：展示文案

`side_caption` 在界面上一律称「展示文案」，不再叫「旁白」。数据库字段名、接口字段名与分析模块的提示词不动——那属于数据契约，改名会牵动迁移与模型输出解析。改动范围只有表单标签、空值占位、按钮提示与列表页搜索框的占位文字。
