...
| 入口 | 方法与路径 | 说明 |
|---|---|---|
| 页面 | `GET/POST /admin/providers` | 一个 `action` 字段区分 create、update、delete、import 四个动作；列表页的启用模型下拉也走 update，只提交 `active_model` 一个字段 |
...
**「启用」与「正在被用」是两件独立的事，列表页必须同时显示。** `is_enabled` 只表示这条档案允许被引用；真正决定谁干活的是配置管理页的三个用途路由。只显示启用状态时，每条档案都写着「启用」却可能一条都没接上，用户会以为建档即生效，然后对着一条根本没被引用的档案排查「分析为什么没走新模型」。因此状态列除启用态外还标注「用于 照片分析、信息面板」这类用途，未被任何路由引用时明确标注「未被引用」；若已有档案但三个路由全空，页面顶部再给一条指向配置管理页的提示条。引用关系由 `ModelProviderService.routes_by_provider()` 一次算出厂商名到路由键的映射，避免每条档案各查一遍路由。
